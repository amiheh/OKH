...
//                    slot[i]=timeslot[i][1];
            ...
            int[][]scurrent = timeslot.clone();
            int[][]stemp = timeslot.clone();
            ...
            while(!terminate){
                iteration++;
                    ...
                    //choose candidate
                    double random_sol = Math.floor(Math.random()*5);
                    stemp = sneighborhood.get((int)random_sol);

                    current_temp = current_temp * reduction_fact;

                    penalty1 = penalty(conflict_matrix, sbest, student.size());
                    penalty2 = penalty(conflict_matrix, scurrent, student.size());
                    penalty3 = penalty(conflict_matrix, stemp, student.size());

                    if (penalty3 <= penalty2) {
                        scurrent = stemp;
                        penalty2 = penalty3;
                        if (penalty3 <= penalty1) {
                            sbest = stemp;
                            penalty1 = penalty3;
                        }
                    }
                    else if (Math.exp((penalty2-penalty3)/current_temp) > Math.random()) {
                        scurrent = stemp;
                        penalty2 = penalty3;
                    };
                ...
                sbest_array.add(sbest);
                sbest_penalty[iteration-1] = penalty3;
            ...
            System.out.println("penalty terbaik dari SA: "+ minPenalty);

            System.out.println("max timeslot : " + max_timeslot);
            
            export(sbest,filename);
            export_trajectory(trajectory,filename);
    ...
    public static void export_trajectory(double[] trajectory, String filename){
        try{    
            FileWriter fw=new FileWriter("C:\\TORONTO\\"+filename+"_SA.txt");    
            for (int i = 0; i < trajectory.length; i++) {
            	fw.write(trajectory[i]+ "\n");
            }
            ...
            System.out.println("File trajectory "+filename+"_SA.txt berhasil disimpan di C");    
    }
